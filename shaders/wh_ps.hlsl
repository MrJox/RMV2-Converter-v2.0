...
float4 ps_main( in VS_OUTPUT input ) : SV_TARGET
{
	...
	float4 specular = t_specular.Sample(s_default, input.TexCoord.xy) * 3.0f;
	...
	float4 diff = diffuseFactor * diffuse;
	float4 spec = specFactor * specular;

	float4 litColour = diff + spec * smoothness;
